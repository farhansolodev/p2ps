...
func runUDPServer(port int) {
        ...
        if cm != nil {
                ...
                fmt.Printf("[Port %d] Received on interface %s (idx=%d), dst=%v\n",
                        port, ifaceName, cm.IfIndex, cm.Dst)
        } else {
                fmt.Printf("[Port %d] WARNING: cm is nil!\n", port)
        }
            ...
            fmt.Printf("[Port %d] Sending reply: Src=%v, IfIndex=%d\n", port, replyCM.Src, replyCM.IfIndex)
